...
export class WindowsConPtyBackend implements TerminalSessionBackend {
	...
	async ensureSession(
		...
		if (!existsSync(this.helperPath)) {
			throw new Error(
				`Terminal host executable was not found. Checked: ${this.helperPathCandidates.join(", ")}. Run the helper build first.`,
			);
		}
	...
	private createHelperPathCandidates(): string[] {
		...
		const helperSegments = [
			...
			"Debug",
			"net10.0-windows",
			`TerminalWindowManager.ConPTYHost${extension}`,
		];
	...
	private async handleHelperLine(
		...
		try {
			...
			await this.emitSessionError(terminal, liveSession, {
				...
				message: `The terminal host emitted invalid JSON: ${line}`,
	...
	private async handleChildProcessError(
		...
		await this.emitSessionError(terminal, liveSession, {
			type: "error",
			sessionId: liveSession.sessionId,
			message: `The terminal host process failed: ${error.message}`,
			diagnosticLogPath: liveSession.eventsPath,
